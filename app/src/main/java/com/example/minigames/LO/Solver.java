    ...
    private void init() {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                Set<Coord> filteredSwitchedCoords = null;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    filteredSwitchedCoords = switchedCoords.stream()
    ...
    public Solutions solve(int maxSolutions) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                // et non vérouillées
                Set<Equation> matchedEquations = null;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    matchedEquations = notLockedEquations.stream()
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            impossible = this.equations.stream()
        ...
        for (int step = 0; step < nbSolutionsToCompute; step++) {
            ...
            List<Equation> equationsSet = null;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                equationsSet = this.equations.stream()
